...
trait LoadViewView
{
    public function MakeLoadViewView()
    {
        $d = $this->getDefaults();
        $name = $d['name'];
        $nameUp = ucfirst($name);
        ...
        if (!is_dir($path)) {
            ...
            File::makeDirectory($path."\\loads\\", 0755, true);
        }

        $filePath =  $path ."\\". $viewFile . '.blade.php';
        if (!File::exists($filePath)) {
            $content = $this->LoadViewViewFileSting($model,$pageDotUrl);
            File::put($filePath, $content);
            $this->info("View " . $viewFile . " created ");
        }
        $filePath =  $path ."\\loads\\". $name . '.blade.php';
        if (!File::exists($filePath)) {
            $content = "Loded Data";
            File::put($filePath, $content);
            $this->info("Load file  " . $name . " created ");
        }
    ...
    public function LoadViewViewFileSting($model)
    {
                    ...
                    <div id="defaultPage" class="pages">
                        <div class="card rounded-0 pb-3">
                            ...
                            <div class="mt-2 p-2 p-md-4">
                                <form id="frm{$nameUp}" autocomplete="off">
                                        ...
                                        <div class="col-md-4 offset-md-4">
                                            ...
                                            <div class="mb-3 mt-3 text-end">
                                                <button class="btn btn-primary btn-sm" type="submit">
                                                    ...
                                                    {{Lang::get('common.btns.glob_load')}}
                                                </button>
                                            </div>
                                        </div>
                                    </div>
