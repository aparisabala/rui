...
use File;
use Str;
trait ReactCreateView
{
    public function MakeReactView()
    {
        $d = $this->getDefaults();
        $name = $d['name'];
        $model = $d['model'];
        ...
        if (!File::exists($filePath)) {
            $content = $this->ReactViewFileSting($model,$pageDotUrl);
        ...
        if($pageNav == "yes") {
            ...
            if (!File::exists($filePath)) {
                $content = <<<PHP
                <a href="{{ url('{$pageUrl}/manage') }}" class="p-2 p-link">{$name}</a>
                PHP;
                File::put($filePath, $content);
    ...
    public function ReactViewFileSting($model)
    {
        $string = implode(" ",explode("_",Str::snake($model)));
        $d = $this->getDefaults();
        $properNameSpace = implode("/",explode("\\", $d['properNameSpace']));
        $pageNav = $d['pageNav'];
        $name = $d['name'];
        ...
        $codeString = <<<PHP
        @extends('{$first}.layout.main_layout',
            [
                "tabTitle" => config('i.service_name')." |  B1 | B2 | B3",
                "react" => [
                    "{$properNameSpace}/{$name}.js"
                ]
            ]
        )
                ...
                <div class="col-md-12">
                    <div id="defaultPage" class="pages">
                        <div class="card rounded-0 pb-3">
                            {{-- pageNav --}}
                            <h2 class="text-center text-uppercase fs-20 m-0 p-0 pt-2">  MainTitle </h2>
                            <hr class="m-0 mt-2">
