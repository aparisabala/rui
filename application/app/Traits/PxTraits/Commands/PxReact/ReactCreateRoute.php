...
use File;
use Str;
trait ReactCreateRoute
{
    public function MakeReactRoute()
    {
        $d = $this->getDefaults();
        $model = $d['model'];
        $formType = $d['formType'];
        ...
        if (!File::exists($filePath)) {
            $content = $this->ReactRouteAssetsString($name,$properNameSpace,$pageUrl,$formType);
            File::put($filePath, $content);
            $this->info("Route ".Str::camel($name)." created");
        }
    }

    public function ReactRouteAssetsString($name,$nameSpace,$url,$formType)
    {
