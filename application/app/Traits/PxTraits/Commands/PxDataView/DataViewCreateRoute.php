...
use File;
use Str;
trait DataViewCreateRoute
{
    public function MakeDataViewRoute()
    {
        $d = $this->getDefaults();
        $model = $d['model'];
        $formType = $d['formType'];
        ...
        if (!File::exists($filePath)) {
            $content = $this->DataViewRouteAssetsString($name,$properNameSpace,$pageUrl,$formType);
            File::put($filePath, $content);
            $this->info("Route ".Str::camel($name)." created");
        }
    }

    public function DataViewRouteAssetsString($name,$nameSpace,$url,$formType)
    {
